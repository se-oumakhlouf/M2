 ...
 *
 * The method {@link #repaint(ColorPlus)} (Color)} can be used to set a
 * background color.
 *
 *
 * The mouse events can be retrieved using
 * {@link #waitForMouseEvents(MouseCallback)}.
 *
...
public class CoolGraphics {
	...
	final LinkedBlockingQueue<MouseEvent> eventBlockingQueue = new LinkedBlockingQueue<>();

	public enum ColorPlus {
		RED(Color.RED), GREEN(Color.GREEN), ORANGE(Color.ORANGE), BLUE(Color.BLUE), BLACK(Color.BLACK), WHITE(Color.WHITE);
	 ...
	 * {@link MouseCallback#mouseClicked(int, int)} will be called for each mouse
	 * event until the window is {@link #close() closed}.
	 *
	 * @param mouseCallback a mouse callback.
	 *
	 * @throws IllegalStateException if this method is called by the event dispatch
	 *                               thread.
	 */
	public void waitForMouseEvents(MouseCallback mouseCallback) {
		...
		for (;;) {
			...
			if (mouseEvent.getX() < border || mouseEvent.getY() < border || mouseEvent.getX() >= area.getWidth() - 2 * border
					|| mouseEvent.getY() >= area.getHeight() - 2 * border) {
				continue;
			}
	...
	private static final MouseEvent CLOSE_EVENT = new MouseEvent(new JButton(), -1, -1, -1, 0, 0, 0, 0, 0, false, 0);
